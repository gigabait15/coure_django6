...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Настройка рассылки с ID {id} не найдена")
            )
            return
        ...
        for client in setting.client.all():
            ...
            mail_server_response = (
                f"Письмо '{setting.message.letter_subject}' успешно отправлено"
            )
            try:
                send_mail(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Попытка отправки писем для настройки с ID {id} завершена"
            )
        )
