...
class MessageMailingDetailView(
...
class MessageMailingUpdateView(
...
class MessageMailingDeleteView(
    ...
    def test_func(self):
        obj = self.get_object()
        return (
            self.request.user == obj.client
            or self.request.user.has_perm("MailingSetting.delete_mailing_setting")
...
class MailingSettingDetailView(
    LoginRequiredMixin, PermissionRequiredMixin, generic.DetailView
):
...
class MailingSettingCreateView(
...
class MailingSettingUpdateView(
    LoginRequiredMixin, PermissionRequiredMixin, generic.UpdateView
):
    model = MailingSetting
    form_class = MailingSettingForm
...
class MailingSettingDeleteView(
    LoginRequiredMixin, UserPassesTestMixin, generic.DeleteView
):
    model = MailingSetting
    success_url = reverse_lazy("MailingSetting:mailing_list")
