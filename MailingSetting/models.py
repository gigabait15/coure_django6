...
class MessageMailing(models.Model):
    ...
    client = models.ForeignKey(
        ServiceClient, on_delete=models.CASCADE, verbose_name="Клиент сервиса"
    )
...
class MailingSetting(models.Model):
    ...
    status = models.CharField(
        choices=STATUS_CHOICES, default="создана", max_length=20, verbose_name="Статус"
    )
    client = models.ManyToManyField(ServiceClient, verbose_name="Клиент сервиса")
    message = models.OneToOneField(
        MessageMailing, on_delete=models.CASCADE, verbose_name="Сообщение рассылки"
    )
...
class MailingAttempt(models.Model):
    ...
    last_attempt_datatime = models.CharField(
        max_length=200, verbose_name="дата и время последней попытки"
    )
    attempt_status = models.CharField(
        choices=ATTEMPT_CHOICES, max_length=200, verbose_name="статус попытки "
    )
    mail_server_response = models.TextField(
        verbose_name="ответ почтового сервера", **NULLABLE
    )

    mailingsetting = models.ForeignKey(
        MailingSetting, on_delete=models.CASCADE, verbose_name="настройка рассылки"
    )
